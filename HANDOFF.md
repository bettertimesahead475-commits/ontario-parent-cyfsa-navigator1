...
**Tests:** `api/services/matterLegalDiscovery.test.ts` -- 19 tests passed (context derivation, classification preservation, no-caller-ID discovery, VERIFIED-only surfacing, unverified-source exclusion/limitation, allegation-stays-allegation, spoofed-trust-text rejected, prompt-injection-as-data, deterministic ranking, deterministic tie-break, tokenize normalization, candidate creation, candidate reuse/no-duplicate, full-orchestration persistence, ranking-explanation persistence, matter isolation reject, safe-empty-result, ranking-never-changes-trust).

**Limitations (explicit, not silently assumed away):** no legal-issue-mapping table exists in this repository yet, so discovery is pure term overlap over provision label/citation text, not issue-classification matching; a provision whose only VERIFIED version does not (yet) resolve to a specific event date is still discovered/ranked but its candidate carries `legalSourceVersionId: null` and `contentIntegrityStatus: NOT_CHECKED`/`UNVERIFIED` per Stage 9B's own existing rules -- this service never invents a version resolution Stage 9B itself would refuse. Result volume is capped (`maxResults`, default 25) with deterministic rank-order truncation.

**Deferred to 9D-2b (explicitly not started here):** Express routes/endpoint handlers, request/response payload/schema design, per-endpoint authentication/authorization wiring, IDOR tests, any UI, case-brief/work-product integration via `navigator_matter_research_work_product_links`.

**Milestone status: STAGE 9D-2A IMPLEMENTED — AWAITING REVIEW.** Stage 9A, 9B, 9C and 9D-1 remain independently frozen/closed as recorded above and are unaffected by this entry -- no existing service file, test, or the 9D-1 migration was modified. Stage 9D as a whole is **not** complete; 9D-2b, 9D-3 and 9D-4 remain to be implemented.
