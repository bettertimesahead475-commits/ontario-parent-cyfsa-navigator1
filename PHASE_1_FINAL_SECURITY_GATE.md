...
4. Current status: **PARTIALLY FIXED** (was: FIXED per prior docs; corrected here after independent verification).
5. Blocks Phase 1: No — this is an onboarding/documentation gap, not a live vulnerability (the app fails loudly/safely with a 503 and a descriptive error if this var is missing, rather than silently misbehaving).
6. Recommended action: add one line to `.env.example`: `FIREBASE_SERVICE_ACCOUNT_JSON=` with the same descriptive comment style as its neighbors. Trivial, should be folded into the next docs pass.
...
- **`.env.example`**: contains no real secret values — every sensitive variable is blank or a placeholder (`GEMINI_API_KEY="MY_GEMINI_API_KEY"`, etc.), confirmed by direct read. **Gap found this pass**: `FIREBASE_SERVICE_ACCOUNT_JSON` is missing entirely (see L-1 above) — not a leaked secret, but a documentation gap for a required variable.
...
1. **`.env.example` still missing `FIREBASE_SERVICE_ACCOUNT_JSON`** (L-1, corrected status: PARTIALLY FIXED) — low severity, onboarding-only, the app fails loudly rather than silently without it.
...
3. **Add `FIREBASE_SERVICE_ACCOUNT_JSON` to `.env.example`** — trivial, should happen before or alongside the next piece of work touching this file.
