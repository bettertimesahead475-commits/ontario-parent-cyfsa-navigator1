 ...
 * create_case_with_owner() Postgres function (see the migration file for why
 * this - rather than two separate INSERTs from here - is what guarantees a
 * case can never exist without an owner).
 *
...
export async function createCase(ownerUid: string, title: string): Promise<Case> {
  ...
  const { data, error } = await db.rpc("create_case_with_owner", {
