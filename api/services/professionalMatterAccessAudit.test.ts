...
const created = (over: Record<string, unknown> = {}) => ({
  ...
  status: 'PENDING', expires_at: '2026-10-01T00:00:00Z', created_at: '2026-09-24T00:00:00Z', ...over,
});

beforeEach(() => {
  calls = [];
  vi.spyOn(accounts, 'findAccount').mockResolvedValue({ id: 'acct-owner', primaryRole: 'parent', status: 'active' } as any);
  rpc = vi.fn(async (fn: string) => fn === 'navigator_matter_access_lifecycle_contract_v3'
    ? { data: ACCESS_LIFECYCLE_CONTRACT, error: null }
    : { data: created(), error: null });
});

describe('createProfessionalGrant (contract v3, atomic create_matter_grant)', () => {
  it('requires the v3 contract and uses exactly one lifecycle RPC; no direct table write', async () => {
    expect(ACCESS_LIFECYCLE_CONTRACT).toBe('navigator_matter_access_lifecycle_v3');
    const { grant, rawToken } = await createProfessionalGrant('uid-owner', MATTER);
    expect(calls).toEqual(['navigator_matter_access_lifecycle_contract_v3', 'create_matter_grant']);
    expect(grant).toMatchObject({ id: created().id, matterId: MATTER, status: 'PENDING' });
    const args = rpc.mock.calls[1][1];
    expect(args).toEqual({ p_firebase_uid: 'uid-owner', p_matter_id: MATTER, p_token_digest: crypto.createHash('sha256').update(rawToken).digest('hex'), p_expires_in_days: 7 });
    ...
    expect(Object.keys(args).sort()).toEqual(['p_expires_in_days', 'p_firebase_uid', 'p_matter_id', 'p_token_digest']);
  });
  ...
  it.each([
    ...
    rpc.mockImplementation(async (fn: string) => fn === 'navigator_matter_access_lifecycle_contract_v3' ? contract : { data: created(), error: null });
    await expect(createProfessionalGrant('uid-owner', MATTER)).rejects.toThrow(/access lifecycle contract/);
    expect(calls).toEqual(['navigator_matter_access_lifecycle_contract_v3']);
  });

  it.each([[0], [-1], [366], [1.5]])('rejects expiry %s days before any database call', async days => {
    await expect(createProfessionalGrant('uid-owner', MATTER, { expiresInDays: days })).rejects.toThrow(/1 to 365 days/);
  ...
  it.each([
    ...
    rpc.mockImplementation(async (fn: string) => fn === 'navigator_matter_access_lifecycle_contract_v3'
      ? { data: ACCESS_LIFECYCLE_CONTRACT, error: null } : { data: null, error: { message } });
    const err = await createProfessionalGrant('uid-owner', MATTER).catch(e => e);
  ...
  it.each([
    ...
    await expect(createProfessionalGrant('uid-owner', MATTER)).rejects.toThrow(/Failed to create grant/);
  });

  it('accepts a matter id in any case (PostgreSQL canonicalizes uuids)', async () => {
    await expect(createProfessionalGrant('uid-owner', MATTER.toUpperCase())).resolves.toBeTruthy();
  });
});
